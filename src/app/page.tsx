...
const projects: Project[] = [
  {
    ...
    demo: "https://aero-flight-tracker-mlc.streamlit.app/",
...
export default function Home() {
  ...
  const handleSendMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "assistant", content: data.content || "Je n'ai pas pu répondre." },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
  ...
  return (
    <div className="min-h-screen bg-black text-zinc-100 selection:bg-emerald-500 selection:text-black overflow-x-hidden">
      <main className="px-4 py-8 sm:px-6 md:px-16 md:py-12 max-w-6xl mx-auto">
        {/* En-tête */}
        <header className="flex flex-col md:flex-row md:items-center justify-between border-b border-zinc-900 pb-8 mb-12 gap-6">
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <a
              ...
              className="flex-1 sm:flex-initial flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-zinc-900 border border-zinc-800 hover:border-emerald-500 hover:text-white transition-all text-xs font-semibold text-zinc-300 hover:shadow-[0_0_12px_rgba(16,185,129,0.2)]"
            >
        ...
        {/* Profil & Compétences (Radar SVG responsive) */}
        <section className="mb-16">
          <h2 className="text-2xl font-bold text-white tracking-tight mb-8">
            Profil & Compétences
          </h2>

          <div className="p-3 sm:p-8 md:p-12 rounded-3xl bg-zinc-950/80 border border-zinc-900 flex justify-center items-center overflow-hidden">
            <div className="relative w-full max-w-[620px] aspect-[16/11] flex items-center justify-center">
              <svg className="w-full h-full" viewBox="-320 -180 640 360">
        ...
        {/* Widget Flottant Sharpex avec sécurité anti-débordement */}
        <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 max-w-[calc(100vw-2rem)]">
          {isChatOpen ? (
            <div className="w-[calc(100vw-2rem)] sm:w-[380px] h-[480px] sm:h-[520px] bg-zinc-950 border border-zinc-800 rounded-2xl shadow-2xl flex flex-col overflow-hidden animate-in fade-in slide-in-from-bottom-5 duration-200">
